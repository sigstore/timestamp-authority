...
import (
	...
	"github.com/sigstore/timestamp-authority/pkg/pki/x509/testutils"
...
func TestCertChain_Verify(t *testing.T) {
	caSigner, err := signer.New(context.Background(), "memory")
	if err != nil {
		t.Fatal(err)
	}
	// A properly created cert chain should encode to PEM OK.
	ctx := context.Background()
	mem, err := signer.New(context.Background(), "memory")
	if err != nil {
		t.Fatal(err)
	}
