...
PROVIDER_NAME="sigstore-timestamp"
LOCATION="global"
REPO="sigstore/timestamp-authority"
SERVICE_ACCOUNT_ID="github-actions-timestamp"
...
if ! (gcloud iam workload-identity-pools providers describe "${PROVIDER_NAME}" --location="${LOCATION}" --workload-identity-pool="${POOL_NAME}"); then
  ...
  --display-name="Github Actions Provider Timestamp Authority" \
...
gcloud iam service-accounts create ${SERVICE_ACCOUNT_ID} \
  --description="Service account for Github Actions Timestamp Authority" \
  --display-name="Github Actions Timestamp Authority"
fi
